...
test_attribute_string_encodings(void)
{
    ...
    if ((type_id1 = H5Tcopy(H5T_C_S1)) < 0) {
        H5_FAILED();
        HDprintf("    couldn't copy builtin string datatype\n");
        goto error;
    }
    ...
    if ((H5Tset_cset(type_id1, H5T_CSET_ASCII)) < 0) {
        H5_FAILED();
        HDprintf("    couldn't set character set of string to ASCII\n");
        goto error;
    }
    ...
    if ((dset_id1 = H5Dcreate(container_group, ATTRIBUTE_STRING_ENCODINGS_DSET_NAME1, type_id1, space_id,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        HDprintf("    couldn't create dataset with ascii string\n");
        goto error;
    }
    ...
    if ((attr_id1 = H5Acreate(dset_id1, ATTRIBUTE_STRING_ENCODINGS_ATTR_NAME1, type_id1, space_id,
                              H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        HDprintf("    couldn't create attribute with ascii string\n");
        goto error;
    }
    ...
    if ((H5Tset_cset(type_id2, H5T_CSET_UTF8)) < 0) {
        H5_FAILED();
        HDprintf("    couldn't set character set of string to UTF-8\n");
        goto error;
    }
    ...
    if ((dset_id2 = H5Dcreate(container_group, ATTRIBUTE_STRING_ENCODINGS_DSET_NAME2, type_id2, space_id,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        H5_FAILED();
        HDprintf("    couldn't create dataset with UTF-8 string\n");
        goto error;
    }
    ...
    BEGIN_MULTIPART
    {
        PART_BEGIN(ASCII_cset)
        {
            ...
            if ((write_buf = calloc(1, ascii_str_size + 1)) == NULL) {
                H5_FAILED();
                HDprintf("    couldn't allocate memory for write buffer\n");
                goto error;
            }
            ...
            if (H5Awrite(attr_id1, type_id1, write_buf) < 0) {
                H5_FAILED();
                HDprintf("    couldn't write to attribute with ASCII string\n");
                goto error;
            }

            if (H5Aread(attr_id1, type_id1, read_buf) < 0) {
                H5_FAILED();
                HDprintf("    couldn't read from attribute with ASCII string\n");
                goto error;
            }

            if (strncmp(write_buf, read_buf, ascii_str_size)) {
                H5_FAILED();
                HDprintf("    incorrect data read from attribute with ASCII string\n");
                goto error;
            }
        ...
        PART_BEGIN(UTF8_cset)
        {
            ...
            if (H5Awrite(attr_id2, type_id2, write_buf) < 0) {
                H5_FAILED();
                HDprintf("    couldn't write to attribute with UTF-8 string\n");
                goto error;
            }

            if (H5Aread(attr_id2, type_id2, read_buf) < 0) {
                H5_FAILED();
                HDprintf("    couldn't read from attribute with UTF-8 string\n");
                goto error;
            }

            if (strncmp(write_buf, read_buf, utf8_str_size)) {
                H5_FAILED();
                HDprintf("    incorrect data read from attribute with UTF-8 string\n");
                goto error;
            }
